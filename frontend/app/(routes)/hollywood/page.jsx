...
export async function generateMetadata() {
  return {
    ...
    description: `download hollywood hindi dubbed for free in 480p,720p,1080p and 4K and hindi dubbed in vegamovies | filmyzila | hdmovies`,
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_URL}/hollywood`,
    },
    robots: {
      index: true,
      follow: true,
    ...
    <>
      <section>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 gap-y-10 mt-16 items-start">
          {data.length === 0
            ? redirect(`/hollywood?page=1`)
            : data.map((d, index) => <MovieCard data={d} key={index} />)}
        </div>
        <div className="flex items-center justify-center gap-5">
          <PageRouter page={page} url="hollywood" />
        </div>
      </section>
